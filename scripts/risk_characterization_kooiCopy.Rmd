...
spatial_risk_thresholds <- sfBay_aligned %>% 
    mutate(threshold = case_when(particle.L.master.50 < Threshold1_food ~ "< One",
    particle.L.master.50 >= Threshold1_food & particle.L.master.50 < Threshold2_food ~ "> One",
    particle.L.master.50 >= Threshold2_food & particle.L.master.50 < Threshold3_food ~ "> Two",
    particle.L.master.50 >= Threshold3_food & particle.L.master.50 < Threshold4_food ~ "> Three",
    particle.L.master.50 >= Threshold4_food ~ "> Four")) %>% 
  #filter(!sampleID == "CB9 Jan 11 Total") %>% 
  filter(Sampling.apparatus == "Manta Trawl") %>% 
  filter(!str_detect(sampleID, "DUP")) %>% 
  filter(!str_detect(sampleID, "blank")) %>% 
...
```{r include=FALSE}
#color prep
color_scale <- c("< One" = "#b6d7a8", #green
                "> One" = "#fcc523", #yellow
                "> Two" = "#bf8345", #orange
                "> Three" = "#e06665", # red
                "> Four" = "#ac5784") #purple

shape_scale <- c("< One" = 21, #circle
                "> One" = 22, #square
                "> Two" = 23, #diamond
                "> Three" = 24, # upside triangle
                "> Four" = 25) #triangle
