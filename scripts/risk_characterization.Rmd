...
library(purrr)
map(list(ca, ca_co), ~st_crs(.x)$epsg)
...
spatial_risk <- sfBay_aligned %>% 
  ...
  filter(particle.L.master.50 < 100) %>% 
  ...
  st_as_sf(coords = c("longitude_num", "latitude_num"), 
           crs = 4326) #%>%  # California Albers
  #st_transform(crs = 3310)
#inspect
st_crs(spatial_risk)
st_crs(ca_co) # need to transform to CA Albers

mapview(spatial_risk, zcol="particle.L.master.50") # this may take a minute because of duplication
```
...
#let's map shapefiles in ggplot!
risk_smc <- ggplot()+
  geom_sf(data = sf) +
  geom_sf(data = #sfBay_aligned, 
          spatial_risk,
          aes(color = particle.L.master.50), alpha = 0.7,
          size = 5)+
  labs(color = "Concentration")+
  scale_color_viridis_c(option = "viridis") +#good for continuous data
  ...
  ylab("Latitude") +
  ggtitle("Microplastics Concentrations in San Francisco Bay") +
  #coord_sf(xlim = c(), ylim = c()) #range of plot to be included
  theme_bw()
risk_smc
```
