...
manta.data <- particle.data %>% 
  filter(SampleMatrix == "manta") %>% 
...
manta.polymer.simple <- manta.polymer.percentages %>%
  ...
  rename(location = site,
         Polymer = PlasticType,
         proportion = freq) %>% 
  dplyr::select(c(Sample.Type, Polymer, location, proportion, general))


#full dataframe
polymer_site_matrices <- rbind(long_polymer2, manta.polymer.simple) %>% 
  ...
  mutate(Polymer = case_when(
    ...
    Polymer == "Asphalt" ~ "Other anthropogenic (asphalt, rubber, paint, wax)",
...
alignment_factors <- data.frame("matrix" = c("Surface (1L-grab)", "Surface (Manta Trawl)", "Fish", "Storm", "WWTP", "Sediment"),
                                ...
                                "alpha.se" = c(alpha.marine.se, alpha.marine.se, alpha.fish.se, alpha.stormwater.se, alpha.wastewater.se, alpha.sediment.se)) %>% 
...
sensitivity <- sfBay_new2 %>%
  ...
  mutate(conc.upper.68 = case_when(sample.matrix.specific == "surface water" ~ CF.upper.68 * unaligned.particle.L.master,
         ...
         conc.upper.95 =  case_when(sample.matrix.specific == "surface water" ~ CF.upper.95 * unaligned.particle.L.master,
         ...
         conc.lower.95 =  case_when(sample.matrix.specific == "surface water" ~ CF.lower.95 * unaligned.particle.L.master,
