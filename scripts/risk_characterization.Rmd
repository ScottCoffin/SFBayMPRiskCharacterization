...
#new <- left_join(sfBay, dates, by = "sampleID")

#remove grab samples that don't have dates
sfBay_noGrab <- sfBay %>% 
  filter(Sampling.apparatus != "1-L grab")

grab_with_dates_new <- grab_with_dates %>% dplyr::select(-c('Station Code', Date, particle_count, sample_volume_L))
...
```{r}
#see matches
matches <- left_join(sfBay_noGrab, grab_with_dates_new %>%
...
```
```{r}
ggsave(plot = match_plot,
       filename = "match_plot.jpeg",
       path = "output/figures/", 
...
sfBayECDF_sensitivity <- sensitivity %>% 
 ...
 # stat_ecdf(aes(x = value, color = name),
  ...
  # geom_vline(xintercept = Threshold2_food, linetype = 'solid', color = "#ebbb38", size = 1) +
...
```
##### Wet Vs Dry
Note that grab samples don't have dates so can't annotate wet/dry!
```{r}
#ECDF by System
sfBayECDF_wetDry <- sfBay_new %>% 
  ...
  #color
  scale_color_manual(values = hcl(h = c(60,240,60,240),c = 100, l = 65, alpha=c(0.4,0.4,1,1), fixup = TRUE)) +
