...
## PLOTTING ##
  ...
  #aligned
  geom_boxplot(notch = TRUE, width = 1) +
...
```
```{r}
ggsave(plot = SFEI_mesh_matrix,
       filename = "SFEI_mesh_matrix.jpg",
       path = "output/figures/", 
       width = 10, height = 6, units = "in",
       bg = "white",
       dpi = 300)
...
sensitivity.analysis.manta.oxidative <- sensitivity %>% 
  filter(source == "SFEI") %>% 
  filter(sample.matrix.specific == "surface water") %>% 
  filter(Sampling.apparatus == "Manta Trawl") %>%
  #just select concentrations
  ...
  pivot_longer(everything()) %>% 
  mutate(aboveThreshold = factor(case_when(
    value < Threshold1_oxidative.stress ~ "below Threshold 1",
    value >= Threshold1_oxidative.stress & value < Threshold2_oxidative.stress ~ "above Threshold 1",
    value >= Threshold2_oxidative.stress & value < Threshold3_oxidative.stress ~ "above Threshold 2",
    value >= Threshold3_oxidative.stress & value < Threshold4_oxidative.stress ~ "above Threshold 3",
    value >= Threshold4_oxidative.stress ~ "above Threshold 4"
  ))) %>% 
  group_by(aboveThreshold, name) %>%
  dplyr::summarize(n = n()) %>% 
  mutate(rel.freq = paste0(round(100 *(n/manta.count[1,]),1),"%")) %>% 
...
```{r}
sensitivity.analysis.location <- sensitivity %>% 
  filter(source == "SFEI") %>% 
  filter(sample.matrix.specific == "surface water") %>% 
  #just select concentrations
  ...
  group_by(aboveThreshold, name, locationNew) %>%
  dplyr::summarize(n = n()) %>% 
  left_join(count.location, by = "locationNew") %>% 
  mutate(rel.freq = paste0(round(100 *(n/total)),"%")) %>% 
  pivot_wider(names_from = "name", values_from = c("n", "rel.freq")) %>% 
  mutate(aboveThreshold = factor(aboveThreshold, levels = c("below Threshold 1", "above Threshold 1", "above Threshold 2", "above Threshold 3", "above Threshold 4"))) %>% 
  arrange(aboveThreshold)
...
```{r}
sensitivity.analysis.manta.location <- sensitivity %>% 
  filter(source == "SFEI") %>% 
  filter(sample.matrix.specific == "surface water") %>% 
  filter(Sampling.apparatus == "Manta Trawl") %>%
  #just select concentrations
  ...
  group_by(aboveThreshold, name, locationNew) %>%
  dplyr::summarize(n = n()) %>% 
  left_join(manta.count.location, by = "locationNew") %>% 
  mutate(rel.freq = paste0(round(100 *(n/total)),"%")) %>% 
  pivot_wider(names_from = "name", values_from = c("n", "rel.freq")) %>% 
  mutate(aboveThreshold = factor(aboveThreshold, levels = c("below Threshold 1", "above Threshold 1", "above Threshold 2", "above Threshold 3", "above Threshold 4"))) %>% 
  arrange(aboveThreshold)
...
sensitivity.analysis.manta.location
```

###### EXPORT EXCEEDANCES
```{r}
write.csv(sensitivity.analysis.manta.location ,
          "output/data/sensitivity.analysis.manta.location.csv"
          )
```
##### Oxidative Stress
###### MANTA and GRAB
...
```{r}
sensitivity.analysis.location.oxidative <- sensitivity %>% 
  filter(source == "SFEI") %>% 
  filter(sample.matrix.specific == "surface water") %>% 
  #just select concentrations
  dplyr::select(c(Conc, #conc.upper.68,
...
sensitivity.analysis.location.oxidative
```

###### EXPORT EXCEEDANCES
```{r}
write.csv(sensitivity.analysis.location.oxidative,
          "output/data/sensitivity.analysis.location.oxidative.csv"
          )
```
...
```{r}
sensitivity.analysis.manta.location.oxidative <- sensitivity %>% 
  filter(source == "SFEI") %>% 
  filter(sample.matrix.specific == "surface water") %>% 
  filter(Sampling.apparatus == "Manta Trawl") %>%
  #just select concentrations
