...
```{r}
water_correction_factor <- 0.68 #68% of particles were plastic
fish_correction_factor <- 0.23 #23% of particles were plastic in fish

sfBay3 <- sfBay2 %>% 
  mutate(particles.L.blank.corrected.particle.corrected = case_when(
    sample.matrix == "water" ~ particles.L.blank.corrected * water_correction_factor)) %>% 
  mutate(particles.fish.blank.corrected.particle.corrected = case_when(sample.matrix == "fish" ~ particles.fish.blank.corrected * fish_correction_factor))
```
...
sfBay_aligned <- sfBay3 %>% 
  ...
  mutate(particle.L.master = particles.L.blank.corrected.particle.corrected * CF,
         ...
         particles.kg.master = particles.kg.blank.corrected * CF) %>% 
