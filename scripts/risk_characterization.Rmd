...
shape_summary_manta_noFibers


#join manta without fibers and other data
shape_summary <- rbind(simple_shape_summary, shape_summary_manta_noFibers)
shape_summary
```
...
descdist(plastic.proportions.manta, discrete = FALSE)
```
...
fit <- fitDist(plastic.proportions.manta, k = 2, type = "realplus", trace = TRUE, try.gamlss = TRUE)
...
#   )
  ...
  mutate(particle.L.master = particles.L.blank.corrected.particle.corrected.fiber.corrected * CF,
...
```{r}
#prep rest
wide.sensitivity.exceedances <- sensitivity %>% 
  filter(Sampling.apparatus == "Manta Trawl") %>% 
  dplyr::select(c(Conc,
                  conc.combined.upper.95, conc.combined.lower.95, #combined
                  conc.upper.95.proportion, conc.lower.95.proportion, #proportions only
                  conc.upper.95.fiber, conc.lower.95.fiber, #fiber corrections only
                  conc.lower.95, conc.upper.95, #alignment only
                  sampleID)) %>% 
  pivot_longer(-c(sampleID), names_to = "factor", values_to = "conc") %>% 
  mutate(variable = case_when(
    grepl("proportion", factor) ~ "plastic proportion",
    grepl("combined", factor) ~ "combined",
    grepl("fiber", factor) ~ "fiber correction",
    factor == "Conc" ~ "median",
    factor == "conc.lower.95" ~ "alignment",
    factor == "conc.upper.95" ~ "alignment")) %>% 
  mutate(interval = case_when(
    grepl("upper", factor) ~ "upper",
    grepl("lower", factor) ~ "lower",
...
#get median
median <- wide.sensitivity.exceedances$median[4]
...
wide.sensitivity.exceedances.prep <- wide.sensitivity.exceedances %>% 
  ...
  mutate(name = factor(factor(variable), 
                           levels = c("combined", "alignment", "plastic proportion", "fiber correction")))
  ####plot####
## Break axis ##
  ...
  scale_color_manual(name = "Alignment Uncertanties",
                     values = c("Conc" = "#012740",
                              ...
                              #  "conc.upper.68" = "#05619c",
                                "conc.combined.lower.95" = "#8cd3ff",
                                "conc.combined.upper.95" = "#8cd3ff"#,
...
sensitivity.analysis.manta <- sensitivity.analysis.manta_t2 %>% 
  ...
  mutate(Conc.percent = Conc / manta.count[1,],
         conc.lower.95.percent = conc.combined.lower.95 / manta.count[1,],
         conc.upper.95.percent = conc.combined.upper.95 / manta.count[1,])
...
sensitivity.analysis.manta.oxidative_inter <- sensitivity %>% 
  ...
  dplyr::select(c(Conc,# conc.upper.68, 
                  conc.combined.upper.95, 
                  #conc.upper.99, conc.lower.68, 
                  conc.combined.lower.95#, conc.lower.99
                  )) %>% 
  #make long data
  pivot_longer(everything(),names_to = "alignment", values_to = "conc") %>% 
  #mutate(alignment = factor(alignment)) %>% 
...
#fiber.proportions <- shape_fiber$fiber.adjustment
...
```{r}
fiber.proprotions.fit.beta <- fitdist(fiber.proportions, "beta")
plot(fiber.proprotions.fit.beta)
summary(fiber.proprotions.fit.beta)
```
...
```{r}
fit <- fitDist(fiber.proportions, k = 2, type = "realAll", trace = TRUE, try.gamlss = TRUE)
...
colnames(fiber) <- c("fiber.proportion")
  
summary(fiber$fiber.proportion)
fiber %>% ggplot(aes(x = fiber.proportion)) + geom_histogram() + scale_x_log10()
