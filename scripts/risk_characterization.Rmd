...
#calcualted relative standard deviation for concentration factor
  ...
  #calculate alignment relative standard deviation (%) for concentraiont
  mutate(proportion.rsd = proportion.sd / proportion) %>% 
  ## next calculate uncertainty of proportion x alignment
  mutate(combined.rsd = sqrt(CF.rsd ^ 2 + proportion.rsd ^ 2)) %>% 
  #now calculate combined uncertainty
  mutate(conc.combined.sd = Conc * combined.rsd) %>% 
  #now calculate combined CI's
  mutate(conc.combined.upper.95 = Conc + 1.96 * (conc.combined.sd / sqrt(alpha.n)),
         conc.combined.lower.95 = Conc - 1.96 * (conc.combined.sd / sqrt(alpha.n))) %>% 
...
sensitivity %>% 
  ...
  dplyr::select(c(CF.rsd,
                  ...
                  combined.rsd,
                  sampleID)) %>% 
  summarize(CF.rsd = mean(CF.rsd), proportion.rsd = mean(proportion.rsd), combnined.rsd = mean(combined.rsd))
```
...
wide.sensitivity.exceedances <- sensitivity %>% 
  ...
  mutate(variable = case_when(
    grepl("proportion", factor) ~ "proportion",
...
sensitivity.figure <- wide.sensitivity.exceedances %>% 
  filter(variable != "median") %>% 
  #arrange(desc(lower)) %>% 
  mutate(name = factor(factor(variable), 
                           levels = c("combined", "alignment", "proportion"))) %>% 
#plot  
ggplot() +
  geom_errorbarh(aes(y = name, xmin = lower, xmax = upper),
                 na.rm = TRUE) +
  geom_vline(xintercept = median, linetype = "dashed", color = "red", size = 1.2) +
  scale_x_continuous(name = "Percentage of Manta Trawl Samples Exceeding \n Food Dilution Threshold One (% +- 95% CI)",
                       labels = scales::percent_format(scale = 100)) +
