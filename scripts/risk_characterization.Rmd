...
sfBayGrab <- sfBay_aligned %>% 
  ...
  mutate(sampleIDSimple = sampleID, 
         ...
         CF_grab = CF) %>% 
  select(c(sampleIDSimple, particle.L.master_grab, unaligned.particle.L.master_grab, CF_grab))
...
sfBayECDF_alignedXtechnique <- sfBay_new %>% 
  filter(source == "SFEI") %>% 
  filter(sample.matrix.specific == "surface water") %>% 
  select(c(sampleID, unaligned.particle.L.master, particle.L.master, Sampling.apparatus)) %>% 
