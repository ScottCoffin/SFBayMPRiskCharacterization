...
spatial_risk_thresholds <- sfBay_aligned %>% 
    ...
    particle.L.master.50 >= Threshold1_food & particle.L.master.50 < Threshold2_food ~ "Investigative",
    particle.L.master.50 >= Threshold2_food & particle.L.master.50 < Threshold3_food ~ "Source tracking",
    particle.L.master.50 >= Threshold3_food & particle.L.master.50 < Threshold4_food ~ "Impaired waterbody",
    particle.L.master.50 >= Threshold4_food ~ "Pollution control measures")) %>% 
  #filter(!sampleID == "CB9 Jan 11 Total") %>% 
  filter(Sampling.apparatus == "Manta Trawl") %>% 
  filter(!str_detect(sampleID, "DUP")) %>% 
  filter(!str_detect(sampleID, "blank")) %>% 
...
color_scale <- c("< Thresholds" = "#b6d7a8", #green
                "Investigative" = "#fcc523", #yellow
                "Source tracking" = "#bf8345", #orange
                "Impaired waterbody" = "#e06665", # red
                "Pollution control measures" = "#ac5784") #purple

shape_scale <- c("< Thresholds" = 21, #circle
                "Investigative" = 22, #square
                "Source tracking" = 23, #diamond
                "Impaired waterbody" = 24, # upside triangle
                "Pollution control measures" = 25) #triangle
