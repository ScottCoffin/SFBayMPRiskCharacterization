...
water_SFEI_mesh_matrix <- sfBay_aligned %>% 
  ...
  mutate(alignment_pretty = case_when(alignment == "particle.L.master" ~ "Recaled & Corrected",
                                      alignment == "unaligned.particle.L.master" ~ "Unadjusted")) %>% 
## PLOTTING ##
...
finalDataset <- sensitivity %>% 
  ...
  dplyr::select(c(
    #sample details
   Reference, sampleID, sample.matrix, sample.matrix.specific, compartment, Sampling.apparatus,Sample.Type,
   #location details
   ...
   #correction factors
    fiber.correction.factor, proportion, x1M, x2M, alpha, alpha.sd, CF, 
   #corrected concentrations
    particle.L.master.05, particle.L.master.50, particle.L.master.95, particle.L.master, particle.fish.master, particles.kg.master)) %>% 
  rename("min_mesh_size_um" = x1M,
         "max_mesh_size_um" = x2M,
         "plastic_proportion_correction" = proportion,
         "rescaling_correction_factor" = CF,
         "particles.L_rescaled_corrected_5thPercentile" = particle.L.master.05,
         "particles.L_rescaled_corrected_50thPercentile" = particle.L.master.50,
         "particles.L_rescaled_corrected_95thPercentile" = particle.L.master.95,
         "particles.L_rescaled_corrected_deterministic" = particle.L.master)
...
sensitivity.analysis.location.final <- sensitivity.analysis.location[2:7,] %>% 
  #change to numeric
  mutate_at(grep("conc", colnames(.), ignore.case = TRUE), funs(as.numeric)) %>% 
  mutate(CentralBay = paste0(Conc,
                              " (",
                              conc.combined.lower.95,
                              " to ",
                              conc.combined.upper.95,
                              ")"),
         LowerSouthBay = paste0(Conc.1,
                              " (",
                              conc.combined.lower.95.1,
                              " to ",
                              conc.combined.upper.95.1,
                              ")"),
         NationalMarineSanctuary = paste0(Conc.2,
                              " (",
                              conc.combined.lower.95.2,
                              " to ",
                              conc.combined.upper.95.2,
                              ")"),
         SanPabloBay = paste0(Conc.3,
                              " (",
                              conc.combined.lower.95.3,
                              " to ",
                              conc.combined.upper.95.3,
                              ")"),
         SouthBay = paste0(Conc.4,
                              " (",
                              conc.combined.lower.95.4,
                              " to ",
                              conc.combined.upper.95.4,
                              ")"),
         SuisunBay = paste0(Conc.5,
                              " (",
                              conc.combined.lower.95.5,
                              " to ",
                              conc.combined.upper.95.5,
                              ")")
  ) %>% 
    mutate(CentralBayPerc = paste0(round(100 * (Conc / as.numeric(sensitivity.analysis.location[2,2])),0),
                              "% (",
                              round(100* (conc.combined.lower.95 / as.numeric(sensitivity.analysis.location[2,2])),0),
                              "% to ",
                              round(100 * (conc.combined.upper.95 / as.numeric(sensitivity.analysis.location[2,2])),0),
                              "%)"),
         LowerSouthBayPerc = paste0(round(100 * (Conc.1 / as.numeric(sensitivity.analysis.location[2,3])),0),
                              "% (",
                             round(100 * (conc.combined.lower.95.1 / as.numeric(sensitivity.analysis.location[2,3])),0),
                              "% to ",
                              round(100 * (conc.combined.upper.95.1 / as.numeric(sensitivity.analysis.location[2,3])),0),
                              "%)"),
         NationalMarineSanctuaryPerc = paste0(round(100 * (Conc.2 / as.numeric(sensitivity.analysis.location[2,4])),0),
                              "% (",
                              round(100 * (conc.combined.lower.95.2 / as.numeric(sensitivity.analysis.location[2,4])),0),
                              "% to ",
                              round(100 * (conc.combined.upper.95.2 / as.numeric(sensitivity.analysis.location[2,4])),0),
                              "%)"),
         SanPabloBayPerc = paste0(round(100 * (Conc.3 / as.numeric(sensitivity.analysis.location[2,5])),0),
                              "% (",
                              round(100 * (conc.combined.lower.95.3 / as.numeric(sensitivity.analysis.location[2,5])),0),
                              "% to ",
                              round(100 * (conc.combined.upper.95.3 / as.numeric(sensitivity.analysis.location[2,5])),0),
                              "%)"),
         SouthBayPerc = paste0(round(100 * (Conc.4 / as.numeric(sensitivity.analysis.location[2,6])),0),
                              "% (",
                              round(100 * (conc.combined.lower.95.4/ as.numeric(sensitivity.analysis.location[2,6])),0),
                              "% to ",
                              round(100 * (conc.combined.upper.95.4/ as.numeric(sensitivity.analysis.location[2,6])),0),
                              "%)"),
         SuisunBayPerc = paste0(round(100 * (Conc.5/ as.numeric(sensitivity.analysis.location[2,7])),0),
                              "% (",
                              round(100 * (conc.combined.lower.95.5 / as.numeric(sensitivity.analysis.location[2,7])),0),
                              "% to ",
                              round(100 * (conc.combined.upper.95.5 / as.numeric(sensitivity.analysis.location[2,7])),0),
                              "%)")
  )
...
sensitivity.analysis.manta.location.oxidative.final <- sensitivity.analysis.manta.location.oxidative[2:7,] %>% 
  #change to numeric
  mutate_at(grep("conc", colnames(.), ignore.case = TRUE), funs(as.numeric)) %>% 
  #mutate(across(2:conc.combined.upper.95.5), as.numeric(.)) %>% 
  mutate(CentralBay = paste0(Conc,
                              " (",
                              conc.combined.lower.95,
                              " to ",
...
sfBayECDF_wetDry <- sfBay_new %>% 
  ...
  #aligned
  stat_ecdf(aes(x = particle.L.master.50),geom = "step", linetype = 'solid', alpha = 0.7, size = 2.2) +
  stat_ecdf(aes(x = particle.L.master.95),geom = "step", linetype = 'dotted', alpha = 0.6, size = 1.5) +
  stat_ecdf(aes(x = particle.L.master.05),geom = "step", linetype = 'dotted', alpha = 0.6, size = 1.5) +
  #unaligned
...
spatial_risk_thresholds <- sfBay_aligned %>% 
    mutate(threshold = case_when(particle.L.master.50 < Threshold1_food ~ "< Thresholds",
    particle.L.master.50 >= Threshold1_food & particle.L.master.50 < Threshold2_food ~ "Investigative monitoring",
    particle.L.master.50 >= Threshold2_food & particle.L.master.50 < Threshold3_food ~ "Discharge monitoring",
    particle.L.master.50 >= Threshold3_food & particle.L.master.50 < Threshold4_food ~ "Management planning",
    particle.L.master.50 >= Threshold4_food ~ "Source control measures")) %>% 
  #filter(!sampleID == "CB9 Jan 11 Total") %>% 
  filter(Sampling.apparatus == "Manta Trawl") %>% 
  filter(!str_detect(sampleID, "DUP")) %>% 
  filter(!str_detect(sampleID, "blank")) %>% 
...
#get county outline
sf <- ca_co %>% 
  filter(name %in% c("San Francisco", "Alameda", "Contra Costa", "San Mateo", "Solano", "Marin", "Sonoma", "Napa", "Santa Clara"))
...
mapping_data <- spatial_risk_thresholds %>% 
  mutate('particles/L (median)' = round(particle.L.master.50,3),
         'particles/L (5th percentile)' = round(particle.L.master.05,3),
         'particles/L (95th percentile)' = round(particle.L.master.95,3)) %>% 
...
interactive
```
Are the general area classifgicaitons correct?
```{r}
...
usamap +
  geom_point(data = spatial_risk, aes(x = longitude, y = latitude, group = particle.L.master.50))
```
