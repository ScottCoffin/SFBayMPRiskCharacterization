...
The following alpha values are site-specific for SFB and derived in fit_MLE_mantaData.R. Make sure to COMMENT OUT these values if using the Kooi et al values (see below).
## SITE-SPECIFIC
```{r eval=FALSE, include=FALSE}
###### Choose concentration alignment parameters####
alpha.stormwater <- 2.41
alpha.stormwater.sd <- 0.22
alpha.stormwater.n <- 411

alpha.marine <- 2.15#2.00 #based on SFEI dataset
alpha.marine.sd <- 0.48 #based on SFEI dataset
alpha.marine.n <- 1241
#alpha.marine.rsd <- alpha.marine.se / alpha.marine

alpha.wastewater <- 2.41
alpha.wastewater.sd <- 0.22
alpha.wastewater.n <- 411

alpha.sediment <- 2.89
alpha.sediment.sd <- 0.44
alpha.sediment.n <- 238
...
## KOOI ET AL (2021)
The following alpha values are from Kooi et al (2021). Make sure to COMMENT OUT these values if using the SFB site-specific values (see above).
```{r}
###### Choose concentration alignment parameters####
alpha.stormwater <- 2.97
alpha.stormwater.sd <- 0.83
alpha.stormwater.n <- 187

alpha.marine.site <- 2.15 #based on MLE
alpha.marine.sd.site <- 0.48 #based on MLE

alpha.marine <- 2.07 #based on Kooi et al (2021)
alpha.marine.sd <- 0.03 #based on Kooi et al (2021)
alpha.marine.n <- 1241 #based on MLE
#alpha.marine.rsd <- alpha.marine.se / alpha.marine

alpha.wastewater <- 2.54
alpha.wastewater.sd <- 0.01
alpha.wastewater.n <- 504

alpha.sediment <- 2.57
alpha.sediment.sd <- 0.20
alpha.sediment.n <- 238
...
sfBay <- read.csv("data/SFBayData.csv", na.strings = "NA", stringsAsFactors = TRUE) %>%
  ...
  mutate(x1M = case_when(Sampling.apparatus == "1-L grab" ~ 50,
         TRUE ~ as.numeric(Min.particle.size.um)))
```
...
<!--          particle.L.master_grab = particles.L.50, -->
<!--          unaligned.particles.L_grab = unaligned.particles.L, -->
<!--          CF_grab = CF) %>% -->
<!--   dplyr::select(c(sampleIDSimple, particle.L.master_grab, unaligned.particles.L_grab, CF_grab)) -->
...
#bind
sfBay2 <- rbind(sfBay_noGrab %>%  dplyr::select(c(specific.location, System, general.location, latitude, longitude, particles.L.blank.corrected, particles.fish.blank.corrected, particles.kg.blank.corrected, sample.matrix, sample.matrix.specific, Sampling.apparatus, x1M, x2M, Sample.Type, sampleID)), 
                grab_with_dates_new) %>% 
  #annotate wet/dry
  mutate(season = case_when(grepl("Nov|Dec|Jan|Feb|Mar", sampleID, ignore.case = TRUE) ~ "wet",
                            grepl("Aug|Sep", sampleID, ignore.case = TRUE) ~ "dry"))
...
write.csv(manta.site.plastic.percentages,
          "output/data/kooi/manta.site.plastic.percentages.csv")
```
##### GRAPH
```{r}
...
write.csv(pretty_proportions,
          "output/data/kooi/final_plastic_proportion.csv"
          )
```
...
write.csv(tukey.data,
          "output/data/kooi/tukey_data.csv")
...
write.csv(proportions_samples_allMatrices_summary,
          "output/data/kooi/proportions_samples_allMatrices_summary.csv")
...
htmltools::save_html(tree, file="output/figures/kooi/tree.html")
```
...
ggsave(plot = shape.box,
       filename = "shape_box.jpg",
       path = "output/figures/kooi/", 
...
write.csv(shape_summary_final_pretty,
          "output/data/kooi/shape_summary_final_pretty.csv")
```
...
sfBay2_recode <- sfBay2 %>% 
  mutate(locationNew = case_when(
    ...
    general.location == "Treasure Island" ~ "Central Bay",
    general.location == "SPB" ~ "San Pablo Bay",
    general.location == "SC" ~ "general",
    general.location == "SUB" ~ "Susiun Bay",
...
```{r}
#get plastic particle count per sample (for ANOVA)
grab.sample.plastic.percentages <- proportions_samples_allMatrices %>% 
  filter(SampleMatrix == "sw") %>%  #surface water 1-L grab
  mutate(freq = 100 * proportion)
grab.sample.plastic.percentages
```

#### Fit Distributions
```{r}
plastic.proportions.grab <- grab.sample.plastic.percentages$freq / 1000

#determine possible candidates with descdist
cullen.frey.plastic.grab <- descdist(plastic.proportions.grab, discrete = FALSE, boot =1000)
cullen.frey.plastic.grab
```

```{r}
plastic.proprotions.fit.beta.grab <- fitdist(plastic.proportions.grab, "beta")
plot(plastic.proprotions.fit.beta.grab)
summary(plastic.proprotions.fit.beta.grab)
```


```{r}
plastic.func.grab <- function(F){
  success <- FALSE
  while(!success){
    P = rbeta(n = 1,
              shape1 = plastic.proprotions.fit.beta.grab$estimate[1],
              shape2 = plastic.proprotions.fit.beta.grab$estimate[2])
    success <- P < 0.1} #upper limit of 1 (values were divied by ten for compuation above)
    return(P)
  }
  
plastic.grab <- data.frame(plastic.coefficent = numeric(0));

for(i in 1:n){
  X <- plastic.func.grab()
  plastic.grab <- rbind(plastic.grab, X)
}

colnames(plastic.grab) <- c("plastic.proportion")
  
plastic.grab$plastic.proportion <- plastic.grab$plastic.proportion * 10 # correct from transoformation above

summary(plastic.grab$plastic.proportion)
```
...
```{r}
#get plastic particle count per sample (for ANOVA)
wastewater.sample.plastic.percentages <- proportions_samples_allMatrices %>% 
  filter(SampleMatrix == "eff") %>%  #wastewater
  mutate(freq = 100 * proportion)
wastewater.sample.plastic.percentages
```

#### Fit Distributions
```{r}
plastic.proportions.wastewater <- wastewater.sample.plastic.percentages$freq / 1000

#determine possible candidates with descdist
cullen.frey.plastic.wastewater <- descdist(plastic.proportions.wastewater, discrete = FALSE, boot =1000)
cullen.frey.plastic.wastewater
```

```{r}
plastic.proprotions.fit.beta.wastewater <- fitdist(plastic.proportions.wastewater, "beta")
plot(plastic.proprotions.fit.beta.wastewater)
summary(plastic.proprotions.fit.beta.wastewater)
```


```{r}
plastic.func.wastewater <- function(F){
  success <- FALSE
  while(!success){
    P = rbeta(n = 1,
              shape1 = plastic.proprotions.fit.beta.wastewater$estimate[1],
              shape2 = plastic.proprotions.fit.beta.wastewater$estimate[2])
    success <- P < 0.1} #upper limit of 1 (values were divied by ten for compuation above)
    return(P)
  }
  
plastic.wastewater <- data.frame(plastic.coefficent = numeric(0));

for(i in 1:n){
  X <- plastic.func.wastewater()
  plastic.wastewater <- rbind(plastic.wastewater, X)
}

colnames(plastic.wastewater) <- c("plastic.proportion")
  
plastic.wastewater$plastic.proportion <- plastic.wastewater$plastic.proportion * 10 # correct from transoformation above

summary(plastic.wastewater$plastic.proportion)
```
...
```{r}
#get plastic particle count per sample (for ANOVA)
sediment.sample.plastic.percentages <- proportions_samples_allMatrices %>% 
  filter(SampleMatrix == "sed") %>%  #sediment
  mutate(freq = 100 * proportion)
sediment.sample.plastic.percentages
```

#### Fit Distributions
```{r}
plastic.proportions.sediment <- sediment.sample.plastic.percentages$freq / 1000

#determine possible candidates with descdist
cullen.frey.plastic.sediment <- descdist(plastic.proportions.sediment, discrete = FALSE, boot =1000)
cullen.frey.plastic.sediment
```

```{r}
plastic.proprotions.fit.lnorm.sediment <- fitdist(plastic.proportions.sediment, "lnorm")
plot(plastic.proprotions.fit.lnorm.sediment)
summary(plastic.proprotions.fit.lnorm.sediment)
```


```{r}
plastic.func.sediment <- function(F){
  success <- FALSE
  while(!success){
    P = rlnorm(n = 1,
              meanlog = plastic.proprotions.fit.lnorm.sediment$estimate[1],
              sdlog = plastic.proprotions.fit.lnorm.sediment$estimate[2])
    success <- P < 0.1} #upper limit of 1 (values were divied by ten for compuation above)
    return(P)
  }
  
plastic.sediment <- data.frame(plastic.coefficent = numeric(0));

for(i in 1:n){
  X <- plastic.func.sediment()
  plastic.sediment <- rbind(plastic.sediment, X)
}

colnames(plastic.sediment) <- c("plastic.proportion")
  
plastic.sediment$plastic.proportion <- plastic.sediment$plastic.proportion * 10 # correct from transoformation above

summary(plastic.sediment$plastic.proportion)
```
...
```{r}
#get plastic particle count per sample (for ANOVA)
fish.sample.plastic.percentages <- proportions_samples_allMatrices %>% 
  filter(SampleMatrix == "fish") %>%  #fish
  mutate(freq = 100 * proportion)
fish.sample.plastic.percentages
```

#### Fit Distributions
```{r}
plastic.proportions.fish <- fish.sample.plastic.percentages$freq / 1000

#determine possible candidates with descdist
cullen.frey.plastic.fish <- descdist(plastic.proportions.fish, discrete = FALSE, boot =1000)
cullen.frey.plastic.fish
```

```{r}
plastic.proprotions.fit.beta.fish <- fitdist(plastic.proportions.fish, "beta")
plot(plastic.proprotions.fit.beta.fish)
summary(plastic.proprotions.fit.beta.fish)
```


```{r}
plastic.func.fish <- function(F){
  success <- FALSE
  while(!success){
    P = rbeta(n = 1,
              shape1 = plastic.proprotions.fit.beta.fish$estimate[1],
              shape2 = plastic.proprotions.fit.beta.fish$estimate[2])
    success <- P < 0.1} #upper limit of 1 (values were divied by ten for compuation above)
    return(P)
  }
  
plastic.fish <- data.frame(plastic.coefficent = numeric(0));

for(i in 1:n){
  X <- plastic.func.fish()
  plastic.fish <- rbind(plastic.fish, X)
}

colnames(plastic.fish) <- c("plastic.proportion")
  
plastic.fish$plastic.proportion <- plastic.fish$plastic.proportion * 10 # correct from transoformation above

summary(plastic.fish$plastic.proportion)
```

### Report
```{r}
proportions.all <- rbind(plastic.grab %>% mutate(matrix = "grab"),
      plastic.wastewater  %>% mutate(matrix = "wastewater"),
      plastic.sediment %>% mutate(matrix = "sediment"),
      plastic.fish %>% mutate(matrix = "fish"))

#would ordinarily just join thjis to the CF df, but it creates a vector that exceeds memory limits (wtf.)

proportions_probabilistic <-  proportions.all %>% 
  group_by(matrix) %>% 
  summarize(plastic.proportion.05 = quantile(plastic.proportion, 0.05),
            plastic.proportion.50 = quantile(plastic.proportion, 0.50),
            plastic.proportion.95 = quantile(plastic.proportion, 0.95)) 

write.csv(proportions_probabilistic,
          "output/data/proportions_probabilistic.csv")
proportions_probabilistic
```
...
#min.alpha = min.alpha.marine #from Kooi et al 2021
#max.alpha = max.alpha.marine #from Kooi et al 2021

#create distribution of alpha values based on variation in SF Bay dataset
alpha$alpha <- rnorm(n = n, mean = m.alpha, sd = sd.alpha)
alpha <- alpha %>%  mutate(matrix = "surface")
...
```{r}
#### Manta ####
#same as grab so just duplicate df and annotate with x1M#
alpha.grab.df <- alpha %>% 
  mutate(matrix = "grab") %>% 
  mutate(x1M = sfBay %>% 
           filter(Sampling.apparatus == "1-L grab") %>% 
           summarize(x1M = mean(x1M)) %>%
           pull(x1M))

alpha.manta.df <- alpha %>% 
  mutate(matrix = "manta") %>% 
  mutate(x1M = sfBay %>% 
           filter(Sampling.apparatus == "Manta Trawl") %>% 
           summarize(x1M = mean(x1M)) %>%
...
#### STORMWATER ####
#alpha dataframe
alpha.stormwater.df <- data.frame(n = c(1:n),
                    alpha = numeric(n))

m.alpha =  alpha.stormwater
sd.alpha = alpha.stormwater.sd

#create distribution of alpha values 
alpha.stormwater.df$alpha <- rnorm(n = n, mean = m.alpha, sd = sd.alpha)
alpha.stormwater.df <- alpha.stormwater.df %>% 
  mutate(matrix = "stormwater") %>% 
  mutate(x1M = sfBay %>% filter(sample.matrix.specific == "stormwater") %>%  summarize(x1M = mean(x1M)) %>%  pull(x1M))

#define for plotting
alpha5.stormwater <- as.vector(quantile(alpha.stormwater.df$alpha, 0.05)[1])
alpha50.stormwater <- as.vector(quantile(alpha.stormwater.df$alpha, 0.50)[1])
alpha95.stormwater <- as.vector(quantile(alpha.stormwater.df$alpha, 0.95)[1])

#### wastewater ####
#alpha dataframe
alpha.wastewater.df <- data.frame(n = c(1:n),
                    alpha = numeric(n))

m.alpha =  alpha.wastewater
sd.alpha = alpha.wastewater.sd

#create distribution of alpha values 
alpha.wastewater.df$alpha <- rnorm(n = n, mean = m.alpha, sd = sd.alpha)
alpha.wastewater.df <- alpha.wastewater.df %>%  mutate(matrix = "wastewater") %>% 
  mutate(x1M = sfBay %>% filter(sample.matrix.specific == "wastewater") %>%  summarize(x1M = mean(x1M)) %>%  pull(x1M))

#define for plotting
alpha5.wastewater <- as.vector(quantile(alpha.wastewater.df$alpha, 0.05)[1])
alpha50.wastewater <- as.vector(quantile(alpha.wastewater.df$alpha, 0.50)[1])
alpha95.wastewater <- as.vector(quantile(alpha.wastewater.df$alpha, 0.95)[1])

#### sediment ####
#alpha dataframe
alpha.sediment.df <- data.frame(n = c(1:n),
                    alpha = numeric(n))

m.alpha =  alpha.sediment
sd.alpha = alpha.sediment.sd

#create distribution of alpha values 
alpha.sediment.df$alpha <- rnorm(n = n, mean = m.alpha, sd = sd.alpha)
alpha.sediment.df <- alpha.sediment.df %>%  mutate(matrix = "sediment") %>% 
  mutate(x1M = sfBay %>% filter(sample.matrix.specific == "sediment") %>%  summarize(x1M = mean(x1M)) %>%  pull(x1M))

#define for plotting
alpha5.sediment <- as.vector(quantile(alpha.sediment.df$alpha, 0.05)[1])
alpha50.sediment <- as.vector(quantile(alpha.sediment.df$alpha, 0.50)[1])
alpha95.sediment <- as.vector(quantile(alpha.sediment.df$alpha, 0.95)[1])

#### fish ####
#alpha dataframe
alpha.fish.df <- data.frame(n = c(1:n),
                    alpha = numeric(n))

m.alpha =  alpha.fish
sd.alpha = alpha.fish.sd

#create distribution of alpha values 
alpha.fish.df$alpha <- rnorm(n = n, mean = m.alpha, sd = sd.alpha)
alpha.fish.df <- alpha.fish.df %>%  mutate(matrix = "fish") %>% 
  mutate(x1M = sfBay %>% filter(sample.matrix == "fish") %>%  summarize(x1M = mean(x1M)) %>%  pull(x1M))

#define for plotting
alpha5.fish <- as.vector(quantile(alpha.fish.df$alpha, 0.05)[1])
alpha50.fish <- as.vector(quantile(alpha.fish.df$alpha, 0.50)[1])
alpha95.fish <- as.vector(quantile(alpha.fish.df$alpha, 0.95)[1])
```

### Correction Factors
```{r}
#Combine alpha dataframes
alpha.df <- rbind(alpha.grab.df, 
                  alpha.manta.df,
                  alpha.stormwater.df,
                  alpha.wastewater.df,
                  alpha.sediment.df,
...
```{r}
#define for plotting (manta trawl only)
alpha2 <- alpha.df %>% 
  filter(matrix == "manta")
CF5 <- as.vector(quantile(alpha2$CF, 0.05)[1])
CF50 <- as.vector(quantile(alpha2$CF, 0.50)[1])
CF95 <- as.vector(quantile(alpha2$CF, 0.95)[1])
CFsd <- as.vector(sd(alpha2$CF))

CFdf <- data.frame(names = c("5th", "50th", "95th", "s.d."),
                   vals = c(CF5, CF50, CF95, CFsd))
CFdf
```
...
CF_hist <- alpha2 %>% 
  ...
  scale_x_continuous(limits = c(200, 900),
                     breaks = c(round(CF5,0), round(CF50,0), round(CF95,0), #1000,
                                2000)) +
...
### Other Matrices
```{r} 
# would ordinarily do this more elegantly using a join fnx, but it uses more memory than my laptop has (wtf!)

#### FISH ###
#bind with proportions data
fish.probabilistic <- cbind(plastic.fish, alpha.fish.df) %>% 
  #calculate CF
  mutate(CF = CFfnx(a = alpha,
                    x1D = x1D_set,
                    x2D = x2D_set,
  ...
  mutate(combined.corrections = plastic.proportion * CF) %>% 
  mutate(fiber.proportion = NA)

#### sediment ###
#bind with proportions data
sediment.probabilistic <- cbind(plastic.sediment, alpha.sediment.df) %>% 
  #calculate CF
  mutate(CF = CFfnx(a = alpha,
                    x1D = x1D_set,
                    x2D = x2D_set,
...
#### wastewater ###
#bind with proportions data
wastewater.probabilistic <- cbind(plastic.wastewater, alpha.wastewater.df) %>% 
  #calculate CF
  mutate(CF = CFfnx(a = alpha,
                    x1D = x1D_set,
                    x2D = x2D_set,
...
#### grab ###
#bind with proportions data
grab.probabilistic <- cbind(plastic.grab, alpha.grab.df) %>% 
  #calculate CF
  mutate(CF = CFfnx(a = alpha,
                    x1D = x1D_set,
                    x2D = x2D_set,
...
#### stormwater ###
#no proportions data available
stormwater.probabilistic <- alpha.stormwater.df %>%   
  #calculate CF
  mutate(CF = CFfnx(a = alpha,
                    x1D = x1D_set,
                    x2D = x2D_set,
                    ...
                    x2M = x2D_set)) %>% 
  #calculate combined correction factor
  mutate(combined.corrections = CF) %>% 
  mutate(plastic.proportion = NA) %>% 
  mutate(fiber.proportion = NA)

#combine all
combined.all <- rbind(fish.probabilistic,
      sediment.probabilistic,
      wastewater.probabilistic,
      grab.probabilistic,
      stormwater.probabilistic,
      combined.distributions %>%   # manta trawl
        mutate(matrix = "manta") %>% 
        mutate(x1M = 333) %>%  #333 um mesh for manta
        dplyr::select(c(plastic.proportion, n, alpha, matrix, x1M, CF, combined.corrections, fiber.proportion)) 
      ) 

#print correction factors
all_correction_factors_probabilistic <- combined.all %>% 
  group_by(matrix) %>% 
  summarize(x1M = mean(x1M),
            alpha.05 = quantile(alpha, 0.05),
            alpha.50 = quantile(alpha, 0.50),
            alpha.95 = quantile(alpha, 0.95),
            CF.05 = quantile(CF, 0.05),
            CF.50 = quantile(CF, 0.50),
            CF.95 = quantile(CF, 0.95),
            plastic.proportion.05 = quantile(plastic.proportion, 0.05, na.rm = TRUE),
            plastic.proportion.50 = quantile(plastic.proportion, 0.50, na.rm = TRUE),
            plastic.proportion.95 = quantile(plastic.proportion, 0.95, na.rm = TRUE),
            fiber.proportion.05 = quantile(fiber.proportion, 0.05, na.rm = TRUE),
            fiber.proportion.50 = quantile(fiber.proportion, 0.50, na.rm = TRUE),
            fiber.proportion.95 = quantile(fiber.proportion, 0.95, na.rm = TRUE),
            combined.05 = quantile(combined.corrections, 0.05),
            combined.50 = quantile(combined.corrections, 0.50),
            combined.95 = quantile(combined.corrections, 0.95),
            )

#print

write.csv(all_correction_factors_probabilistic,
          "output/data/kooi/all_correction_factors_probabilistic.csv")

all_correction_factors_probabilistic
```
...
sfBay_aligned <- sfBay3 %>% 
  #recode to match above
  mutate(matrix = case_when(
    matrix == "Fish" ~ "fish",
    matrix == "Sediment" ~ "sediment",
    matrix == "Storm" ~ "stormwater",
    matrix == "WWTP" ~ "wastewater",
    matrix == "Surface (1L-grab)" ~ "grab",
    matrix == "Surface (Manta Trawl)" ~ "manta"
  )) %>% 
  #combine with correction factors derived probabilistically above
  left_join(all_correction_factors_probabilistic %>% 
             dplyr::select(-c(x1M)),
            by = "matrix") %>% 
...
# 
  ...
  # mutate(particles.L.05 = case_when(Sampling.apparatus == "Manta Trawl" ~
  #                                                particles.L.blank.corrected * combined.corrections5)) %>% 
  mutate(particles.L.25 = case_when(Sampling.apparatus == "Manta Trawl" ~
                                                 particles.L.blank.corrected * combined.corrections25)) %>%
  # mutate(particles.L.50 = case_when(Sampling.apparatus == "Manta Trawl" ~
  #                                                particles.L.blank.corrected * combined.corrections50,
  #                                      Sampling.apparatus == "1-L grab" ~ particles.L.blank.corrected * CF)) %>% 
    mutate(particles.L.75 = case_when(Sampling.apparatus == "Manta Trawl" ~
                                                 particles.L.blank.corrected * combined.corrections75)) %>%
  # mutate(particles.L.95 = case_when(Sampling.apparatus == "Manta Trawl" ~
  #                                                particles.L.blank.corrected * combined.corrections95)) %>% 
  #probabilistic corrections (no fiber correction)
  mutate(particles.L.05.noFiberCorrection = case_when(Sampling.apparatus == "Manta Trawl" ~
                                                 particles.L.blank.corrected * combined.corrections.noFiberCorrection5)) %>% 
  mutate(particles.L.25.noFiberCorrection = case_when(Sampling.apparatus == "Manta Trawl" ~
                                                 particles.L.blank.corrected * combined.corrections.noFiberCorrection25)) %>% 
  mutate(particles.L.50.noFiberCorrection = case_when(Sampling.apparatus == "Manta Trawl" ~
                                                 particles.L.blank.corrected * combined.corrections.noFiberCorrection50,
                                       Sampling.apparatus == "1-L grab" ~ particles.L.blank.corrected * CF)) %>% 
    mutate(particles.L.75.noFiberCorrection = case_when(Sampling.apparatus == "Manta Trawl" ~
                                                 particles.L.blank.corrected * combined.corrections.noFiberCorrection75)) %>% 
  mutate(particles.L.95.noFiberCorrection = case_when(Sampling.apparatus == "Manta Trawl" ~
                                                 particles.L.blank.corrected * combined.corrections.noFiberCorrection95)) %>% 
### Probabilistic corrections (other matrices)
  ##Fish
  mutate(particles.fish.05 = case_when(matrix == "fish" ~ particles.fish.blank.corrected * combined.05)) %>% 
  mutate(particles.fish.50 = case_when(matrix == "fish" ~ particles.fish.blank.corrected * combined.50)) %>% 
  mutate(particles.fish.95 = case_when(matrix == "fish" ~ particles.fish.blank.corrected * combined.95)) %>% 
  ##Sediment
  mutate(particles.kg.05 = case_when(matrix == "sediment" ~ particles.kg.blank.corrected * combined.05)) %>% 
  mutate(particles.kg.50 = case_when(matrix == "sediment" ~ particles.kg.blank.corrected * combined.50)) %>% 
  mutate(particles.kg.95 = case_when(matrix == "sediment" ~ particles.kg.blank.corrected * combined.95)) %>% 
  #all water
  mutate(particles.L.05 = particles.L.blank.corrected * combined.05) %>% 
  mutate(particles.L.50 = particles.L.blank.corrected * combined.50) %>% 
  mutate(particles.L.95 = particles.L.blank.corrected * combined.95) %>% 
  # 
  # ##stormwater
  # mutate(particles.L.05 = case_when(matrix == "stormwater" ~ particles.L.blank.corrected * combined.05)) %>% 
  # mutate(particles.L.50 = case_when(matrix == "stormwater" ~ particles.L.blank.corrected * combined.50)) %>% 
  # mutate(particles.L.95 = case_when(matrix == "stormwater" ~ particles.L.blank.corrected * combined.95)) %>% 
  # ##wastwater
  # mutate(particles.L.05 = case_when(matrix == "wastewater" ~ particles.L.blank.corrected * combined.05)) %>% 
  # mutate(particles.L.50 = case_when(matrix == "wastewater" ~ particles.L.blank.corrected * combined.50)) %>% 
  # mutate(particles.L.95 = case_when(matrix == "wastewater" ~ particles.L.blank.corrected * combined.95)) %>% 
  # ##grab
  # mutate(particles.L.05 = case_when(matrix == "grab" ~ particles.L.blank.corrected * combined.05)) %>% 
  # mutate(particles.L.50 = case_when(matrix == "grab" ~ particles.L.blank.corrected * combined.50)) %>% 
  # mutate(particles.L.95 = case_when(matrix == "grab" ~ particles.L.blank.corrected * combined.95)) %>% 
  # ##manta
  # mutate(particles.L.05 = case_when(matrix == "manta" ~ particles.L.blank.corrected * combined.05)) %>% 
  # mutate(particles.L.50 = case_when(matrix == "manta" ~ particles.L.blank.corrected * combined.50)) %>% 
  # mutate(particles.L.95 = case_when(matrix == "manta" ~ particles.L.blank.corrected * combined.95)) %>% 

  #baseline corrections
  # mutate(particle.L.master = case_when(Sampling.apparatus == "Manta Trawl" ~ particles.L.50,
  #                                      Sampling.apparatus == "1-L grab" ~  particles.L.blank.corrected * CF,
  ...
  #        particles.fish.50 = particles.fish.blank.corrected.particle.corrected * CF,
  #        particles.kg.50 = particles.kg.blank.corrected.particle.corrected * CF) %>% 
  rename(unaligned.particles.L = particles.L.blank.corrected.particle.corrected) %>% 

  #filter(unaligned.particles.L > 0) %>%  #remove blanks. should convert to 1/2 LOQ...
  mutate(source = "SFEI") %>% 
 ...
 # mutate(particles.L.50 = particle.L.master) %>% 
  mutate_if(is.character, as.factor)
```

###### EXPORT FINAL DATASET
```{r}
#select variables and rename for ease of interpretations
finalDataset <- sfBay_aligned %>% 
   #annotate locations
  mutate(site = case_when(
    grepl("NB", sampleID,ignore.case = FALSE) ~ "North Bay",
    grepl("CB", sampleID,ignore.case = FALSE) ~ "Central Bay",
  ...
  dplyr::select(c(
    #sample details
   Reference, sampleID, Sample.Type, sample.matrix, sample.matrix.specific, compartment, Sampling.apparatus, x1M, x2M, 
   #location details
    site, specific.location, latitude, longitude, season, 
   # unaligned concentrations
   particles.L.blank.corrected, particles.fish.blank.corrected, particles.kg.blank.corrected, 
   #correction factors
   alpha.05, alpha.50, alpha.95,
   CF.05, CF.50, CF.95,
   fiber.proportion.05, fiber.proportion.50, fiber.proportion.95,
   plastic.proportion.05, plastic.proportion.50, plastic.proportion.95,
   combined.05, combined.50, combined.95,
   #corrected concentrations
    particles.L.05, particles.L.50, particles.L.95, 
   particles.fish.05, particles.fish.50, particles.fish.95,
   particles.kg.05, particles.kg.50, particles.kg.95)) %>% 
  rename("min_mesh_size_um" = x1M,
         "max_mesh_size_um" = x2M,
         "plastic_proportion_correction_5thPercentile" = plastic.proportion.05,
         "plastic_proportion_correction_50thPercentile" = plastic.proportion.50,
         "plastic_proportion_correction_95thPercentile" = plastic.proportion.95,
         "rescaling_correction_factor_5thPercentile" = CF.05,
         "rescaling_correction_factor_50thPercentile" = CF.50,
         "rescaling_correction_factor_95thPercentile" = CF.95,
         "fiber_correction_factor_5thPercentile" = fiber.proportion.05,
         "fiber_correction_factor_50thPercentile" = fiber.proportion.50,
         "fiber_correction_factor_95thPercentile" = fiber.proportion.95,
         "combined_correction_factor_5thPercentile" = combined.05,
         "combined_correction_factor_50thPercentile" = combined.50,
         "combined_correction_factor_95thPercentile" = combined.95,
         "particles.L_rescaled_corrected_5thPercentile" = particles.L.05,
         "particles.L_rescaled_corrected_50thPercentile" = particles.L.50,
         "particles.L_rescaled_corrected_95thPercentile" = particles.L.95,
         "particles.fish_rescaled_corrected_5thPercentile" = particles.fish.05,
         "particles.fish_rescaled_corrected_50thPercentile" = particles.fish.50,
         "particles.fish_rescaled_corrected_95thPercentile" = particles.fish.95,
         "particles.kg_rescaled_corrected_5thPercentile" = particles.kg.05,
         "particles.kg_rescaled_corrected_50thPercentile" = particles.kg.50,
         "particles.kg_rescaled_corrected_95thPercentile" = particles.kg.95,
         )
  

write.csv(finalDataset,
          "output/data/kooi/finalDataset.csv"
          )
```
...
water_SFEI_mesh_matrix <- sfBay_aligned %>% 
  filter(source == "SFEI") %>% 
  filter(particles.L.50 >0,
         unaligned.particles.L > 0) %>% 
  mutate(apparatus.matrix = paste0(sample.matrix.specific, " (", Sampling.apparatus,")")) %>% 
  #filter(sample.matrix.specific == "surface water") %>% 
  drop_na(particles.L.50) %>% 
   dplyr::select(c(sampleID, unaligned.particles.L, particles.L.50,apparatus.matrix,
  ...
  mutate(alignment_pretty = case_when(alignment == "particles.L.50" ~ "Recaled & Corrected",
                                      alignment == "unaligned.particles.L" ~ "Unadjusted")) %>% 
## PLOTTING ##
 ...
 # geom_point(position=position_jitterdodge())+
  ...
  scale_color_manual(name = "Alignment",
                     ...
                     labels = c("Corrected & Rescaled (1 to 5,000 μm)",
                                "Unaligned")) +
...
sediment_SFEI_mesh_matrix <- sfBay_aligned %>% 
  filter(source == "SFEI") %>% 
  filter(particles.kg.50 > 0,
         particles.kg.blank.corrected.particle.corrected > 0) %>% 
  mutate(apparatus.matrix = paste0(sample.matrix.specific, " (", Sampling.apparatus,")")) %>% 
  #filter(sample.matrix.specific == "surface water") %>% 
  drop_na(particles.kg.50) %>% 
   dplyr::select(c(sampleID, particles.kg.blank.corrected.particle.corrected, particles.kg.50,apparatus.matrix,
                   #Sampling.apparatus, sample.matrix.specific
                   )) %>% 
  pivot_longer(-c(sampleID, apparatus.matrix,
                  #Sampling.apparatus, sample.matrix.specific
  ...
  #rename for easy plotting
  mutate(alignment_pretty = case_when(alignment == "particles.kg.50" ~ "Aligned",
...
fish_SFEI_mesh_matrix <- sfBay_aligned %>% 
  filter(source == "SFEI") %>% 
  filter(particles.fish.50 >0,
         particles.fish.blank.corrected.particle.corrected > 0) %>% 
  mutate(apparatus.matrix = paste0(sample.matrix.specific, " (", Sampling.apparatus,")")) %>% 
  #filter(sample.matrix.specific == "surface water") %>% 
  drop_na(particles.fish.50) %>% 
   dplyr::select(c(sampleID, particles.fish.blank.corrected.particle.corrected, particles.fish.50,apparatus.matrix,
                   #Sampling.apparatus, sample.matrix.specific
                   )) %>% 
  pivot_longer(-c(sampleID, apparatus.matrix,
                  #Sampling.apparatus, sample.matrix.specific
  ...
  #rename for easy plotting
  mutate(alignment_pretty = case_when(alignment == "particles.fish.50" ~ "Aligned",
...
alignment_matrix <- sfBay_aligned %>% 
  ...
  #rename for easy plotting
  mutate(alignment_pretty = case_when(alignment == "particles.L.50" ~ "Aligned",
                                      alignment == "unaligned.particles.L" ~ "Unaligned"))
...
write.csv(tukey_pre_data,
          "output/data/kooi/tukey_pre_alignment.csv")
...
write.csv(tukey_post_data,
          "output/data/kooi/tukey_post_alignment.csv")
...
dfSFEI_exceedance <- dfSFEI %>% 
  mutate(aboveThreshold = factor(case_when(
    particles.L.50 < Threshold1 ~ "below Threshold 1",
    particles.L.50 >= Threshold1 & particles.L.50 < Threshold2 ~ "above Threshold 1",
    particles.L.50 >= Threshold2 & particles.L.50 < Threshold3 ~ "above Threshold 2",
    particles.L.50 >= Threshold3 & particles.L.50 < Threshold4 ~ "above Threshold 3",
    particles.L.50 >= Threshold4 ~ "above Threshold 4"
  )))

#give summary stat for exceedance
exceedance <- dfSFEI_exceedance  %>%
  filter(Sample.Type == "sample") %>% 
  dplyr::select(c(particles.L.50, aboveThreshold)) %>%
...
alignment_factors <- data.frame("matrix" = c("Surface (1L-grab)", "Surface (Manta Trawl)", "Fish", "Storm", "WWTP", "Sediment"),
                                ...
                                "alpha.site" = c(alpha.marine.site, alpha.marine, alpha.fish, alpha.stormwater, alpha.wastewater, alpha.sediment),
                                "alpha.sd.site" = c(alpha.marine.sd.site, alpha.marine.sd, alpha.fish.sd, alpha.stormwater.sd, alpha.wastewater.sd, alpha.sediment.sd),
  ...
  mutate(alpha.se.site = alpha.sd.site / sqrt(alpha.n)) %>% 
  mutate(alpha.upper.68 = alpha + alpha.se,
         ...
         alpha.upper.95.site = alpha.site + (1.96*alpha.se),
         alpha.lower.95.site = alpha.site - (1.96*alpha.se),
...
sensitivity <- sfBay_new2 %>%
  ...
  mutate(#particles.L.50.upper.68 = case_when(sample.matrix.specific == "surface water" ~ CF.upper.68 * unaligned.particles.L,
         ...
         particles.L.50.upper.95 =  case_when(sample.matrix == "water" ~ CF.upper.95 * particles.L.blank.corrected.particle.corrected.fiber.corrected,
         ...
         #particles.L.50.upper.99 =  case_when(sample.matrix.specific == "surface water" ~ CF.upper.99 * unaligned.particles.L,
         ...
         ## lower intervals
        # particles.L.50.lower.68 = case_when(sample.matrix.specific == "surface water" ~ CF.lower.68 * unaligned.particles.L,
         ...
         particles.L.50.lower.95 =  case_when(sample.matrix == "water" ~ CF.lower.95 * particles.L.blank.corrected.particle.corrected.fiber.corrected,
         ...
         #particles.L.50.lower.99 =  case_when(sample.matrix.specific == "surface water" ~ CF.lower.99 * unaligned.particles.L,
...
#calcualted relative standard deviation for concentration factor
  ...
  mutate(particles.L.50.combined.sd = particles.L.50 * combined.rsd) %>% 
  #now calculate combined CI's
   mutate(particles.L.50.combined.upper.95 = particles.L.50 + 1.96 * (particles.L.50.combined.sd / sqrt(fiber.correction.n)), #use smallest N
          particles.L.50.combined.lower.95 = particles.L.50 - 1.96 * (particles.L.50.combined.sd / sqrt(fiber.correction.n))) %>% 
 #  #recalculate alignment 95% CI's using actual SD
 #mutate(particles.L.50.upper.95.alignment = particles.L.50 + 1.96 * (surface.water.CF.rsd * unaligned.particles.L),
  #      particles.L.50.lower.95.alignment = particles.L.50 - 1.96 * (surface.water.CF.rsd * unaligned.particles.L))# %>%
 #  #calculate proportion 95% CI's
    mutate(particles.L.50.upper.95.proportion = particles.L.50 + 1.96 * (proportion.rsd * particles.L.50)  / sqrt(proportion.n),
           particles.L.50.lower.95.proportion = particles.L.50 - 1.96 * (proportion.rsd * particles.L.50) / sqrt(proportion.n)) %>%
  #Calculate fiber proportion uncertainty
  mutate(particles.L.50.upper.95.fiber = particles.L.50 + 1.96 * (proportion.rsd * particles.L.50)  / sqrt(proportion.n),
           particles.L.50.lower.95.fiber = particles.L.50 - 1.96 * (fiber.correction.rsd * particles.L.50) / sqrt(fiber.correction.n)) 
...
###Export table showing site-specific correction factors
```{r}
...
CFs <- sfBay_new2 %>%
  #derive correction factors
  mutate(CF.upper.95.site = CFfnx(a = alpha.upper.95.site, x1M = x1M, x2M = x2M, x1D = x1D_set, x2D = x2D_set),
        CF.lower.95.site = CFfnx(a = alpha.lower.95.site, x1M = x1M, x2M = x2M, x1D = x1D_set, x2D = x2D_set),
        CF.site = CFfnx(a = alpha.site, x1M = x1M, x2M = x2M, x1D = x1D_set, x2D = x2D_set)
         )

CF_table <- CFs %>% 
  group_by(Sampling.apparatus, matrix) %>% 
  summarize(mesh_size = mean(x1M),
            CF.site = round(mean(CF.site),2), 
            CF.lower.95.site = round(mean(CF.lower.95.site),2),
            CF.upper.95.site = round(mean(CF.upper.95.site),2))

write.csv(CF_table,
          "output/data/kooi/CF_table.csv")
...
sfBay_aligned_sensitivity <- sfBay_aligned %>% 
  ...
  mutate(particles.L.50.fiber.05 = particles.L.blank.corrected * fiber5.sensitivity,
         particles.L.50.fiber.95 = particles.L.blank.corrected * fiber95.sensitivity,
         particles.L.50.plastic.05 = particles.L.blank.corrected * plastic5.sensitivity,
         particles.L.50.plastic.95 = particles.L.blank.corrected * plastic95.sensitivity,
         particles.L.50.CF.05 = particles.L.blank.corrected * CF5.sensitivity,
         particles.L.50.CF.95 = particles.L.blank.corrected * CF95.sensitivity,
         particles.L.50.combined.05 = particles.L.blank.corrected * combined.corrections5,
         particles.L.50.combined.95 = particles.L.blank.corrected * combined.corrections95)
```
...
wide.sensitivity.exceedances <- sfBay_aligned_sensitivity %>% 
  ...
  # only manta trawl
  filter(Sampling.apparatus == "Manta Trawl") %>% 
  dplyr::select(c(particles.L.50,
                  particles.L.50.combined.05, particles.L.50.combined.95, #combined
                  particles.L.50.plastic.05, particles.L.50.plastic.95, #proportions only
                  particles.L.50.fiber.05, particles.L.50.fiber.95, #fiber corrections only
                  particles.L.50.CF.05, particles.L.50.CF.95, #alignment only
                  sampleID)) %>% 
  pivot_longer(-c(sampleID), names_to = "factor", values_to = "particles.L.50") %>% 
  ...
  summarize(n = n(),
            belowThreshold1 = sum(particles.L.50 < Threshold1_food),
            aboveThreshold1 = sum(particles.L.50 > Threshold1_food),
            aboveThreshold2 = sum(particles.L.50 > Threshold2_food),
            aboveThreshold3 = sum(particles.L.50 > Threshold3_food),
            aboveThreshold4 = sum(particles.L.50 > Threshold4_food)) %>% 
  mutate(percent.above.threshold1 = aboveThreshold1 / n) %>% 
  pivot_wider(c(variable),#,interval,percent.above.threshold1),
              names_from = interval, 
              values_from = percent.above.threshold1)
...
wide.sensitivity.exceedances.prep <- wide.sensitivity.exceedances %>% 
  ...
  mutate(name = factor(factor(variable), 
                           levels = c("combined alignments and corrections", "fiber correction", "size rescaling", "plastic proportion")))
  ####plot####
## Break axis ##
#ide.sensitivity.exceedances.prep$facet <- wide.sensitivity.exceedances.prep$upper < 0.5


wide.sensitivity.exceedances.prep
write.csv(wide.sensitivity.exceedances.prep,
          "output/data/kooi/sensitivity_analysis.csv")
```
...
sensitivity.figure <- wide.sensitivity.exceedances.prep %>% 
  ...
  scale_y_discrete(labels = c("combined alignments \n and corrections", "fiber correction", "size rescaling",  "plastic proportion")) +
...
sensitivity.analysis <- sensitivity %>% 
  ...
  dplyr::select(c(particles.L.50,
                  ...
                  particles.L.75
                  #particles.L.50, 
                  #particles.L.50.upper.68, 
                  #particles.L.50.upper.95, 
                  #particles.L.50.upper.99, particles.L.50.lower.68,
                  #particles.L.50.lower.95#,
                  #particles.L.50.lower.99
                  )) %>% 
  #make long data
...
write.csv(sensitivity.analysis ,
          "output/data/kooi/surface_water_exceedances_sensitivity_analysis.csv"
          )
```
...
sfBayECDF_sensitivity <- sfBay_aligned %>% 
  ...
  filter(particles.L.50 > 0) %>% 
  #just select concentrations
  #dplyr::select(c(particles.L.50, particles.L.50.combined.upper.95, particles.L.50.combined.lower.95)) %>% 
  dplyr::select(c(particles.L.50,
                  particles.L.05,
                  particles.L.95,
                  particles.L.25,
...
sfBayECDF_sensitivity_white <- sfBayECDF_sensitivity +
theme.type +
  theme(legend.title = element_blank(),
        legend.position = c(0.1,0.6),
        legend.key.size = unit(0.6, 'cm'))

sfBayECDF_sensitivity_dark <- sfBayECDF_sensitivity +
...
sfBayECDF_sensitivity_white
```
```{r}
ggsave(plot = sfBayECDF_sensitivity_white,
...
sensitivity.analysis.manta_inter <- sfBay_aligned %>% 
  ...
  #make long data
  pivot_longer(everything(),names_to = "alignment", values_to = "particles.L.50") %>% 
...
sensitivity.analysis.manta <- sensitivity.analysis.manta_t2 %>% 
  ...
  mutate(particles.L.05.percent = particles.L.05 / manta.count[1,],
         particles.L.25.percent = particles.L.25 / manta.count[1,],
         particles.L.50.percent = particles.L.50 / manta.count[1,],
         particles.L.75.percent = particles.L.75 / manta.count[1,],
         particles.L.95.percent = particles.L.95 / manta.count[1,]) %>% 
  #pretty formatting
  mutate(percentages = paste0( round(100 * particles.L.50.percent,0),
                              "% (",
                              round( 100 * particles.L.05.percent, 0),
                              "% to ",
                               round(100 * particles.L.95.percent, 0),
                              "%)")) %>% 
  #pretty formatting
  mutate(numbers = paste0(round(particles.L.50),
                              " (",
                              round(particles.L.05, 2),
                              " to ",
                              round(particles.L.95, 2),
                              ")"))
...
write.csv(sensitivity.analysis.manta,
          "output/data/kooi/surface_water_exceedances_sensitivity_analysis_manta.csv"
          )
```
...
sensitivity.analysis.manta.oxidative_inter <- sfBay_aligned %>% 
  ...
  group_by(alignment) %>% 
  summarize(n = n(),
            belowThreshold1 = sum(particles.L.50 < Threshold1_oxidative.stress),
            aboveThreshold1 = sum(particles.L.50 > Threshold1_oxidative.stress),
            aboveThreshold2 = sum(particles.L.50 > Threshold2_oxidative.stress),
            aboveThreshold3 = sum(particles.L.50 > Threshold3_oxidative.stress),
            aboveThreshold4 = sum(particles.L.50 > Threshold4_oxidative.stress))
...
sensitivity.analysis.manta.oxidative <- sensitivity.analysis.manta.oxidative_t2 %>% 
  filter(!aboveThreshold == "n") %>% 
mutate_at(c(2:6), as.numeric) %>% 
  mutate(particles.L.50.percent = particles.L.50 / manta.count[1,],
         particles.L.05.percent = particles.L.05 / manta.count[1,],
         particles.L.95.percent = particles.L.95 / manta.count[1,]) %>% 
    #pretty formatting
  mutate(percentages = paste0( round(100 * particles.L.50.percent,0),
                              "% (",
...
write.csv(sensitivity.analysis.manta.oxidative ,
          "output/data/kooi/surface_water_exceedances_sensitivity_analysis_manta_oxidative.csv"
          )
```
...
barplot <- sensitivity.analysis.manta %>% 
  ...
  mutate(frequency = as.numeric(particles.L.50.percent)) %>% 
  mutate(frequency_lower = as.numeric(particles.L.05.percent)) %>%
  mutate(frequency.25 = as.numeric(particles.L.25.percent)) %>% 
  mutate(frequency.75 = as.numeric(particles.L.75.percent)) %>% 
  mutate(frequency_upper = as.numeric(particles.L.95.percent)) %>% 
  mutate(threshold = case_when(aboveThreshold == "belowThreshold1" ~ "< Threshold One (0.3)",
                               aboveThreshold == "aboveThreshold1" ~ ">= Threshold One (0.3)",
                               aboveThreshold == "aboveThreshold2" ~ ">= Threshold Two (3.0)",
                               aboveThreshold == "aboveThreshold3" ~ ">= Threshold Three (5.0)",
                               aboveThreshold == "aboveThreshold4" ~ ">= Threshold Four (34)")) %>% 
  ggplot(aes(x = factor(threshold, level = c("< Threshold One (0.3)",
                                             ">= Threshold One (0.3)",
                                             ">= Threshold Two (3.0)",
                                             ">= Threshold Three (5.0)",
                                             ">= Threshold Four (34)")))) +
  ...
  scale_fill_manual(values = c("< Threshold One (0.3)" = "#b6d7a8", #green
                                ">= Threshold One (0.3)" = "#fcc523", #yellow
                               ">= Threshold Two (3.0)" = "#bf8345", #orange
                                ">= Threshold Three (5.0)" = "#e06665", # red
                               ">= Threshold Four (34)" = "#ac5784" #purple
                               )) +
...
#dark mode
barplot_dark <- barplot +
   #95th
  geom_errorbar(aes(ymin = frequency_lower, ymax = frequency_upper),
                linetype = "dashed", width = 1.0, alpha = 0.7, color = "white") +
  #25th and 75th percetnile
  geom_errorbar(aes(ymin = frequency.25, ymax = frequency.75),
                linetype = "solid", width = 0.5, alpha = 1, color = "white") +
  dark_theme_bw(base_size = 15) +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5),
        axis.title.y = element_blank(),
...
```{r}
bar_cdf <- ggarrange(barplot_white, sfBayECDF_sensitivity_white,
          nrow = 2,
          labels = c("A", "B"))
...
####### Dark MODE
```{r}
bar_cdf_dark <- ggarrange(barplot_dark, sfBayECDF_sensitivity_dark,
          nrow = 2,
          labels = c("A", "B"))

bar_cdf_dark

ggsave(plot = bar_cdf_dark,
       filename = "bar_cdf_dark.jpeg",
       path = "output/figures/kooi/", 
       width = 10, height = 8, units = "in",
       bg = "black",
       dpi = 300)
...
sensitivity.analysis.location_inter <- sfBay_aligned %>% 
  ...
  #dplyr::select(-particles.L.50) %>% 
  rename(particles.L.50 = particles.L.50,
         particles.L.50.combined.upper.95 = particles.L.95,
         particles.L.50.combined.lower.95 = particles.L.05) %>% 
  
  filter(source == "SFEI") %>% 
  filter(sample.matrix.specific == "surface water") %>% 
  filter(Sampling.apparatus == "Manta Trawl") %>%
  #just select concentrations
  dplyr::select(c(particles.L.50, #particles.L.50.upper.68,
                  particles.L.50.combined.upper.95, 
                  #particles.L.50.upper.99, particles.L.50.lower.68, 
                  particles.L.50.combined.lower.95, 
                  #particles.L.50.lower.99,
                  locationNew)) %>% 
  #make long data
  pivot_longer(starts_with("particles.L.50"), names_to = "alignment", values_to = "particles.L.50") %>% 

  #pivot_longer(-c(locationNew),names_to = "alignment", values_to = "particles.L.50") %>% 
  #mutate(alignment = factor(alignment)) %>% 
...
sensitivity.analysis.location <- sensitivity.analysis.location_t2# %>% 
 ...
 # mutate_at(c(2:4), as.numeric) %>% 
  #mutate(particles.L.50.percent = particles.L.50 / manta.count[1,],
   #      particles.L.50.lower.95.percent = particles.L.50.lower.95 / manta.count[1,],
    #     particles.L.50.upper.95.percent = particles.L.50.upper.95 / manta.count[1,])
  

sensitivity.analysis.location.final <- sensitivity.analysis.location[2:7,] %>% 
  #change to numeric
  mutate_at(grep("particles.L.50", colnames(.), ignore.case = TRUE), funs(as.numeric)) %>% 
  mutate(CentralBay = paste0(particles.L.50,
                              " (",
                              particles.L.50.combined.lower.95,
                              " to ",
                              particles.L.50.combined.upper.95,
                              ")"),
         LowerSouthBay = paste0(particles.L.50.1,
                              " (",
                              particles.L.50.combined.lower.95.1,
                              " to ",
                              particles.L.50.combined.upper.95.1,
                              ")"),
         NationalMarineSanctuary = paste0(particles.L.50.2,
                              " (",
                              particles.L.50.combined.lower.95.2,
                              " to ",
                              particles.L.50.combined.upper.95.2,
                              ")"),
         SanPabloBay = paste0(particles.L.50.3,
                              " (",
                              particles.L.50.combined.lower.95.3,
                              " to ",
                              particles.L.50.combined.upper.95.3,
                              ")"),
         SouthBay = paste0(particles.L.50.4,
                              " (",
                              particles.L.50.combined.lower.95.4,
                              " to ",
                              particles.L.50.combined.upper.95.4,
                              ")"),
         SuisunBay = paste0(particles.L.50.5,
                              " (",
                              particles.L.50.combined.lower.95.5,
                              " to ",
                              particles.L.50.combined.upper.95.5,
                              ")")
  ) %>% 
    mutate(CentralBayPerc = paste0(round(100 * (particles.L.50 / as.numeric(sensitivity.analysis.location[2,2])),0),
                              "% (",
                              round(100* (particles.L.50.combined.lower.95 / as.numeric(sensitivity.analysis.location[2,2])),0),
                              "% to ",
                              round(100 * (particles.L.50.combined.upper.95 / as.numeric(sensitivity.analysis.location[2,2])),0),
                              "%)"),
         LowerSouthBayPerc = paste0(round(100 * (particles.L.50.1 / as.numeric(sensitivity.analysis.location[2,3])),0),
                              "% (",
                             round(100 * (particles.L.50.combined.lower.95.1 / as.numeric(sensitivity.analysis.location[2,3])),0),
                              "% to ",
                              round(100 * (particles.L.50.combined.upper.95.1 / as.numeric(sensitivity.analysis.location[2,3])),0),
                              "%)"),
         NationalMarineSanctuaryPerc = paste0(round(100 * (particles.L.50.2 / as.numeric(sensitivity.analysis.location[2,4])),0),
                              "% (",
                              round(100 * (particles.L.50.combined.lower.95.2 / as.numeric(sensitivity.analysis.location[2,4])),0),
                              "% to ",
                              round(100 * (particles.L.50.combined.upper.95.2 / as.numeric(sensitivity.analysis.location[2,4])),0),
                              "%)"),
         SanPabloBayPerc = paste0(round(100 * (particles.L.50.3 / as.numeric(sensitivity.analysis.location[2,5])),0),
                              "% (",
                              round(100 * (particles.L.50.combined.lower.95.3 / as.numeric(sensitivity.analysis.location[2,5])),0),
                              "% to ",
                              round(100 * (particles.L.50.combined.upper.95.3 / as.numeric(sensitivity.analysis.location[2,5])),0),
                              "%)"),
         SouthBayPerc = paste0(round(100 * (particles.L.50.4 / as.numeric(sensitivity.analysis.location[2,6])),0),
                              "% (",
                              round(100 * (particles.L.50.combined.lower.95.4/ as.numeric(sensitivity.analysis.location[2,6])),0),
                              "% to ",
                              round(100 * (particles.L.50.combined.upper.95.4/ as.numeric(sensitivity.analysis.location[2,6])),0),
                              "%)"),
         SuisunBayPerc = paste0(round(100 * (particles.L.50.5/ as.numeric(sensitivity.analysis.location[2,7])),0),
                              "% (",
                              round(100 * (particles.L.50.combined.lower.95.5 / as.numeric(sensitivity.analysis.location[2,7])),0),
                              "% to ",
                              round(100 * (particles.L.50.combined.upper.95.5 / as.numeric(sensitivity.analysis.location[2,7])),0),
                              "%)")
  )


sensitivity.analysis.location.final
```
 

###### EXPORT EXCEEDANCES
```{r}
write.csv(sensitivity.analysis.location.final ,
          "output/data/kooi/sensitivity.analysis.location.csv"
          )
```
...
sensitivity.analysis.manta.location.oxidative.final <- sensitivity.analysis.manta.location.oxidative[2:7,] %>% 
  #change to numeric
  mutate_at(grep("particles.L.50", colnames(.), ignore.case = TRUE), funs(as.numeric)) %>% 
  #mutate(across(2:particles.L.50.combined.upper.95.5), as.numeric(.)) %>% 
  mutate(CentralBay = paste0(particles.L.50,
                              " (",
                              particles.L.50.combined.lower.95,
                              " to ",
...
sensitivity.analysis.manta.location.oxidative.final
```
###### EXPORT EXCEEDANCES
```{r}
write.csv(sensitivity.analysis.manta.location.oxidative.final ,
          "output/data/kooi/sensitivity.analysis.manta.location.oxidative.csv"
          )
```
...
sfBayECDF_sensitivity_grab_manta <- sensitivity %>% 
  ...
  filter(particles.L.50 > 0) %>% 
  #just select concentrations
  #dplyr::select(c(particles.L.50, particles.L.50.plus1, particles.L.50.plus2, particles.L.50.plus3, particles.L.50.minus1, particles.L.50.minus2, particles.L.50.minus3)) %>% 
  #make long data
 # pivot_longer(everything()) %>% 
  #mutate(name = as.factor(name)) %>% 
  ggplot() +
 # stat_ecdf(aes(x = value, color = name),
  #          geom = "point", size = 1, alpha = 0.5) +
  stat_ecdf(aes(x = particles.L.50, color = Sampling.apparatus),geom = "step", linetype = 'solid', alpha = 0.7, size = 2.5) +
  #color
  # scale_color_manual(name = "Alignment Uncertanties",
  #                    values = c("particles.L.50" = "#e8ffff",
  #                               "particles.L.50.minus1" = "#bfe6ff",
  #                               "particles.L.50.plus1" = "#bfe6ff",
  #                               "particles.L.50.minus2" = "#8cd3ff",
  #                               "particles.L.50.plus2" = "#8cd3ff",
  #                               "particles.L.50.minus3" = "#009dff",
  #                               "particles.L.50.plus3" = "#009dff"),
  #                    labels = c("Mean", 
  #                               "68th Percentile",
  #                               "68th Percentile",
  #                               "95th Percentile",
...
sfBayECDF_wetDry <- sfBay_new %>% 
  ...
  #aligned
  stat_ecdf(aes(x = particles.L.50),geom = "step", linetype = 'solid', alpha = 0.7, size = 2.2) +
  stat_ecdf(aes(x = particles.L.95),geom = "step", linetype = 'dotted', alpha = 0.6, size = 1.5) +
  stat_ecdf(aes(x = particles.L.05),geom = "step", linetype = 'dotted', alpha = 0.6, size = 1.5) +
  #unaligned
  #stat_ecdf(aes(x = unaligned.particles.L),geom = "step", linetype = 'solid', alpha = 0.2, size = 2.5) +
  #color
...
write.csv(sensitivity.analysis.manta_season,
          "output/data/kooi/sensitivity_analysis_manta_season.csv")
...
```{r}
 sfBay_aligned %>% 
  ...
  group_by(confidence) %>% 
  summarize(n = n(),
            above50 = sum(particles.L.50 > 50), #values from Everaert et al (2020)
            above0.255 = sum(particles.L.50 > 0.255),
            above56 = sum(particles.L.50 > 56),
            above1e6 = sum(particles.L.50 > 1E-6),
            above0.0679 = sum(particles.L.50 > 0.0679)) %>% 
...
sfBayECDF_sensitivity.noFiberCorrection <- sfBay_aligned %>% 
  ...
  #just select concentrations
  #dplyr::select(c(particles.L.50, particles.L.50.combined.upper.95, particles.L.50.combined.lower.95)) %>% 
  dplyr::select(c(particles.L.50.noFiberCorrection,
                  particles.L.05.noFiberCorrection,
                  particles.L.95.noFiberCorrection,
                  particles.L.25.noFiberCorrection,
                  particles.L.75.noFiberCorrection)) %>% 
  #make long data
  pivot_longer(everything()) %>% 
  mutate(name = as.factor(name)) %>% 
  mutate(percentile = case_when(grepl("50", name) ~ "50th Percentile",
...
sensitivity.analysis.manta.noFiberCorrection <- sensitivity.analysis.manta_t2.noFiberCorrection %>% 
  filter(!aboveThreshold == "n") %>% 
  mutate_at(c(2:6), as.numeric) %>% 
  mutate(particles.L.05.percent = particles.L.05.noFiberCorrection / manta.count[1,],
         particles.L.25.percent = particles.L.25.noFiberCorrection / manta.count[1,],
         particles.L.50.percent = particles.L.50.noFiberCorrection / manta.count[1,],
         particles.L.75.percent = particles.L.75.noFiberCorrection / manta.count[1,],
         particles.L.95.percent = particles.L.95.noFiberCorrection / manta.count[1,])
...
dfSFEI_exceedance %>% 
 # filter(particles.L.50 > 0) %>% 
  filter(Sample.Type == "sample") %>% 
  ggplot(aes(x = particles.L.50, fill = aboveThreshold))+
...
sfBayECDF <- sfBay_new %>% 
  ...
  filter(source == "SFEI") %>% 
  drop_na(particles.L.50) %>% 
  filter(particles.L.50 > 0.0) %>% 
    filter(System %in% c("Ocean", "Estuary")) %>% 
  ggplot() +
     #unaligned
  stat_ecdf(aes(x = unaligned.particles.L, color = "Unaligned"),
            geom = "point", size = 1, alpha = 0.5) +
  stat_ecdf(aes(x = unaligned.particles.L, color = "Unaligned"),geom = "step", linetype = 'solid', alpha = 0.3, size = 1) +
  #aligned
  stat_ecdf(aes(x = particles.L.50, color = "Aligned"),
            geom = "point", size = 1, alpha = 0.5) +
  stat_ecdf(aes(x = particles.L.50, color = "Aligned"),geom = "step", linetype = 'solid', alpha = 0.3, size = 1) +
  #color
  scale_color_manual(name = "alignment",
                     values = vals) +
  # ##Food Dilution thresholds
...
sfBayECDF_alignedXtechnique <- sfBay_new %>% 
  ...
  filter(particles.L.50 > 0) %>% 
  filter(sample.matrix.specific == "surface water") %>% 
  dplyr::select(c(sampleID, unaligned.particles.L, particles.L.50, Sampling.apparatus)) %>% 
...
all_data <- sfBay_aligned %>%
  ...
  mutate(particles.any = case_when(
    sample.matrix == "water" ~ particles.L.50,
    sample.matrix == "fish" ~ particles.fish.50,
    sample.matrix == "sediment" ~ particles.kg.50
    ))
...
sfBay_aligned %>% 
  ...
  filter(!Sampling.apparatus == "1-L grab") %>% 
  mutate(scaled.particles.L.50 = scale(particles.L.50),
         scaled.fish.master = scale(particles.fish.50),
         scaled.particles.kg.50 = scale(particles.kg.50)) %>% 
  mutate(particles.any.scaled = case_when(
    sample.matrix == "water" ~ scaled.particles.L.50,
    sample.matrix == "fish" ~ scaled.fish.master,
    sample.matrix == "sediment" ~ scaled.particles.kg.50
    )) %>% 
  drop_na(Sampling.location) %>% 
...
# convert point data
spatial_risk_thresholds <- sfBay_aligned %>% 
    mutate(threshold = case_when(particles.L.50 < Threshold1_food ~ "< Thresholds",
    particles.L.50 >= Threshold1_food & particles.L.50 < Threshold2_food ~ "Investigative monitoring",
    particles.L.50 >= Threshold2_food & particles.L.50 < Threshold3_food ~ "Discharge monitoring",
    particles.L.50 >= Threshold3_food & particles.L.50 < Threshold4_food ~ "Management planning",
    particles.L.50 >= Threshold4_food ~ "Source control measures")) %>% 
  #filter(!sampleID == "CB9 Jan 11 Total") %>% 
  filter(Sampling.apparatus == "Manta Trawl") %>% 
  filter(!str_detect(sampleID, "DUP")) %>% 
  filter(!str_detect(sampleID, "blank")) %>% 
  #drop_na(c(particles.L.50, latitude, longitude)) %>% 
   mutate(latitude = case_when(
    sampleID == "SPB3 Aug 21" ~ 38.03, #probably a typo...
    sampleID == "GFNMS24 Mar 29" ~ 37.976, #missing tag
    sampleID == "GFNMS25 Mar 30" ~ 37.962, #separate these points a little for viz
    sampleID == "MBNMS30 Mar 31" ~ 37.672,#separate these points a little for viz
    sampleID == "CB8 Jan 11" ~ 37.752, #separate these points a little for viz
    sampleID == "SUB1 Aug 21" ~38.114, #separate these points a little for viz
    sampleID == "CB9 Jan 11 Total" ~ 37.71,
    sampleID == "CB5 Nov 5" ~ 37.845,

    TRUE ~ as.numeric(as.character(latitude))
  )) %>%
  mutate(longitude = case_when(
    sampleID == "GFNMS24 Mar 29" ~ -122.932, #missing
    sampleID == "GFNMS25 Mar 30" ~ -122.938, #separate these points a little for viz
    sampleID == "CB8 Aug 25" ~ -122.222, #overlap
    sampleID == "SUB1 Aug 21" ~ -122.050, #overlap
    sampleID == "CB5 Nov 5" ~ -122.40,
...
```{r include=FALSE}
#color prep
color_scale <- c("< Thresholds" = "#b6d7a8", #green
                "Investigative monitoring" = "#fcc523", #yellow
                "Discharge monitoring" = "#bf8345", #orange
                "Management planning" = "#e06665", # red
                "Source control measures" = "#ac5784") #purple

shape_scale <- c("< Thresholds" = 21, #circle
                "Investigative monitoring" = 22, #square
                "Discharge monitoring" = 23, #diamond
                "Management planning" = 24, # upside triangle
                "Source control measures" = 25) #triangle

#get county outline
##USABoundaries is stupid and has two identical column names. must manually remove otherwise DPLYR won't work.
ca_co <- ca_co[-13]

sf <- ca_co %>% 
  dplyr::filter(name %in% c("San Francisco", "Alameda", "Contra Costa", "San Mateo", "Solano", "Marin", "Sonoma", "Napa", "Santa Clara"))

spatial_risk_thresholds$threshold

#view(spatial_risk_thresholds)
```

##Interactive Map
```{r}
mapping_data <- spatial_risk_thresholds %>% 
  mutate('particles/L (median)' = round(particles.L.50,3),
         'particles/L (5th percentile)' = round(particles.L.05,3),
         'particles/L (95th percentile)' = round(particles.L.95,3)) %>% 
   dplyr::select(c(sampleID,
                  Sampling.location,
                  season,
                  'particles/L (5th percentile)',
                  'particles/L (median)',
                  'particles/L (95th percentile)',
                  threshold,
                  longitude,
                  latitude
                  )) 

interactive <- mapview(mapping_data, zcol="threshold", col.regions = color_scale, layer.name = "Threshold",
                       map.types = c("Esri.WorldShadedRelief", "OpenStreetMap.DE", "CartoDB.Positron", "CartoDB.DarkMatter", "OpenStreetMap", "Esri.WorldImagery", "OpenTopoMap"))


#htmlwidgets::saveWidget(widget = interactive, file = "output/map.html", selfcontained = T)
mapshot(interactive,
         url = paste0(getwd(), "/output/map.html"))

interactive
```
Are the general area classifications correct?
```{r}
sanctuary <- mapping_data %>% 
  filter(Sampling.location == "National Marine Sancturay",
         threshold == "Investigative monitoring")
...
```{r include=FALSE}
library(ggmap)
#register with googlemaps through scoff003@ucr.edu
## NOTE: necessary to create 90-d free trial with Google Cloud, enable billing, and add Geocoding, Geolocaiton, and Static Map APIs#
register_google(key = "AIzaSyDzFuDj1gAUNk6bXnoLMfRPoNvoHtKNW4Y", write= TRUE)

#SF basemap
sf_basemap <- get_map(location = c(lon = -122.352541, lat = 37.838441),
                      zoom = 9, maptype = 'terrain-background', source = 'stamen')
ggmap(sf_basemap)

sf_basemap_watercolor <- get_map(location = c(lon = -122.352541, lat = 37.838441), zoom = 9, maptype = 'watercolor', source = 'stamen')
ggmap(sf_basemap_watercolor)

sf_basemap_terrain_bw <- get_map(location = c(lon = -122.352541, lat = 37.838441), zoom = 9, maptype = 'terrain-background', source = 'google', color = "bw")
ggmap(sf_basemap_terrain_bw)

sf_basemap_toner_background <- get_map(location = c(lon = -122.352541, lat = 37.838441), zoom = 9, maptype = 'toner-background', source = 'google', color = "bw")

ggmap(sf_basemap_toner_background)
```
## Ggplot
```{r}
## assign labels ##
...
ggsave(plot = risk_smc_dark,
       filename = "risk_smc_dark.jpeg",
       path = "output/figures/kooi/", 
       width = 9, height = 8, units = "in",
       bg = "black",
       dpi = 300)
...
##Intersect with Integrated Report Regions
```{r}
library(spatialEco)
#import data#
IR_shp <- st_read(dsn = "data/IR_shp/CalWQA_GeoWQA_Poly_Excerpt.shp")

#ensure polygons are valid
st_is_valid(IR_shp)
#need to make valid
IR_shp <- st_make_valid(IR_shp)

#now ensure CRS is same between points and polygons
IR_shp <- st_set_crs(IR_shp, 4326)

#intersect polygons and points
#intersect <- st_intersection(IR_shp, mapping_data) #keeps points geomtry, but we want polygons preserved

intersect <- st_intersection(mapping_data, IR_shp)
intersect <- as.data.frame(intersect) %>% 
   mutate(threshold1 = case_when(
    particles.L..median. > 0.3 ~ 1,
    particles.L..median. < 0.3 ~ 0
  )) %>% 
  dplyr::select(c(threshold1, OBJECTID))

# Reformat regions based on portion exceedance #
int_sum <- int_poly %>% 
  group_by(OBJECTID) %>% 
  summarize(above = sum(threshold1),
            total = n(),
            ratio = above/total)

int_poly <- left_join(int_sum, IR_shp, by = "OBJECTID") %>% 
  st_as_sf()
...
#### Regions with points
```{r}
IR_risk_map <- ggmap(sf_basemap_toner_background) + 
  #add labels for bay
geom_sf_text(aes(label = name),
              data = labels,
              inherit.aes = FALSE,
  ...
  ##Add monitoring daata
   geom_sf(data = #sfBay_aligned, 
           ...
           #shape = 21,
           color = "gray40") +
  #add color scale for thresholds
  scale_color_manual(name = "Threshold", values = color_scale) +
  scale_fill_manual(name = "Threshold", values = color_scale) +
  scale_shape_manual(name = "Threshold", values = shape_scale) +
  
  coord_sf(crs = st_crs(4326)) + #force CRS to be 4326
  annotation_scale(location = "bl") + #add scale
  annotation_north_arrow(location = "bl", #north arrow
                         pad_x =  unit(0.5, "in"), pad_y = unit(0.5, "in"), #shift it so it doesn't plot on top of scale bar
                         style = north_arrow_fancy_orienteering()) + #change style of north arrow
...
#save
ggsave(plot = IR_risk_map,
       filename = "IR_risk_map.jpeg",
       path = "output/figures/kooi/", 
       width = 9, height = 8, units = "in",
       bg = "white",
       dpi = 300)
...
#### Regions with ratios
```{r}
IR_risk_map_poly <- ggmap(sf_basemap_toner_background) + 
  coord_sf(crs = st_crs(4326)) + #force CRS to be 4326
  #add labels for bay
geom_sf_text(aes(label = name),
              data = labels,
...
#save
ggsave(plot = IR_risk_map_poly,
       filename = "IR_risk_map_poly.jpeg",
       path = "output/figures/kooi/", 
       width = 9, height = 8, units = "in",
       bg = "white",
       dpi = 300)
...
usamap +
  geom_point(data = spatial_risk, aes(x = longitude, y = latitude, group = particles.L.50))
```
